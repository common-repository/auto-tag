...
class AutoTagSetup
{
    ...
    public function __construct()
    {
        ...
        add_action('admin_init', array($this, 'tools_handler'));
    }
...
<div class="icon32"><img src="'.plugin_dir_url( __FILE__ ).'/images/auto_tags.png'.'" /></div><h2>';
        ...
        if(isset($_SESSION['offset'])) {
            echo '<p><b>'.$_SESSION['offset'].'</b> posts/pages have been re-tagged on <b>'.date('Y-m-d', $_SESSION['retag_date']).'</b></p>';
        }
...
<div class="icon32"><img src="'.plugin_dir_url( __FILE__ ).'/images/auto_tags.png'.'" /></div><h2>';
    ...
    public function tools_handler()
    {
        ...
        if(isset($_POST['submit'])) {
            $_SESSION['offset'] = 0;
            $_SESSION['retag_date'] = time();
        }
        if(isset($_SESSION['offset'])) {
            $offset = filter_var($_SESSION['offset'], FILTER_VALIDATE_INT);
        }


        if ($_REQUEST['submit'] == __('Retag all posts', 'auto-tag')) {
            $args = array(
                'post_type' => 'post',
                'post_status' => 'publish',
                'numberposts' => 999,
        ...
        }elseif ($_REQUEST['submit'] == __('Retag all pages', 'auto-tag')) {
            $args = array(
                'post_type' => 'page',
                'post_status' => 'publish',
                'numberposts' => 999,
        ...
        foreach($posts as $p) {
            ...
            if($now > $start + $max_time) {
                ...
                header('Location: http://'.$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'].'?page=auto_tag_options_tools_menu&action=tools&submit='.$_REQUEST['submit']);
            }
            $post = (array) $p;
            $post['post_ID'] = $p->ID;
            $post['action'] = 'save';
